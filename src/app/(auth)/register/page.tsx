...
import { faEye, faEyeSlash, faSpinner, faBrain, faArrowRight, faArrowLeft } from "@fortawesome/free-solid-svg-icons";
  ...
  return (
    <div className="bg-background text-foreground h-screen max-h-screen flex flex-col lg:flex-row select-none overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-[45%] bg-card/25 border-r border-border/40 relative flex-col justify-between p-12 overflow-hidden h-full">
        ...
        <div className="relative z-10 space-y-8 my-auto text-left">
          ...
          <h2 className="text-4xl md:text-5xl font-black text-white leading-[1.1] tracking-tight">
            Start tracking <br />
            <span className="text-primary">your spending habits.</span>
          </h2>
          
          <p className="text-text-secondary text-sm leading-relaxed max-w-sm">
            Gabung dengan ribuan pengguna lain untuk merancang prioritas anggaran hidup nyata Anda dan mengunci kebiasaan belanja yang logis.
          </p>
          ...
          <div className="bg-card/75 border border-border/80 p-5 rounded-[22px] shadow-xl space-y-4 max-w-sm relative">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white text-xs font-bold">12K+ Portfolios Tracked</p>
                <p className="text-text-secondary text-[10px]">Real-time behavioral updates</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-between py-6 px-6 lg:px-12 h-full relative overflow-hidden">
        ...
        <div className="w-full max-w-[400px] mx-auto my-auto bg-card/60 backdrop-blur-xl border border-white/5 rounded-[28px] p-6 md:p-8 shadow-2xl relative z-10 max-h-[calc(100vh-80px)] overflow-y-auto scrollbar-thin">
          ...
          <div className="text-center mb-5">
            ...
            <div className="flex justify-center items-center gap-2 mt-2">
              <span className={`text-[9px] font-black uppercase tracking-wider ${step === 1 ? "text-primary" : "text-text-secondary"}`}>1. Profile</span>
              <div className="w-6 h-[1px] bg-border/60"></div>
              <span className={`text-[9px] font-black uppercase tracking-wider ${step === 2 ? "text-primary" : "text-text-secondary"}`}>2. Security</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-text-secondary ml-4" htmlFor="name">
                    Full Name
                  </label>
                  <input
                    {...register("name")}
                  ...
                  />
                  {errors.name && (
                    <p className="text-[11px] text-red-400 font-bold ml-4 mt-0.5">{errors.name.message}</p>
                  )}
                </div>
                ...
                <button
                  type="button"
                  onClick={handleNext}
                  className="w-full h-11 bg-primary text-primary-foreground text-xs rounded-full font-bold transition-all transform active:scale-95 shadow-lg shadow-primary/10 hover:brightness-105 flex items-center justify-center gap-2 mt-6 cursor-pointer"
                >
                  Continue <FontAwesomeIcon icon={faArrowRight} className="w-3 h-3" />
                </button>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-text-secondary ml-4" htmlFor="confirmPassword">
                    Confirm Password
                  </label>
                  <div className="relative">
                    <input
                    ...
                    <button
                      ...
                      type="button"
                    >
                      {showConfirmPassword ? <FontAwesomeIcon icon={faEyeSlash} className="w-4 h-4" /> : <FontAwesomeIcon icon={faEye} className="w-4 h-4" />}
                    </button>
                  </div>
                  {errors.confirmPassword && (
                    <p className="text-[11px] text-red-400 font-bold ml-4 mt-0.5">{errors.confirmPassword.message}</p>
                  )}
                </div>
                ...
                <div className="flex gap-3 mt-6">
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="flex-1 h-11 border border-border text-white text-xs rounded-full font-bold transition-all transform active:scale-95 hover:bg-white/5 flex items-center justify-center gap-2 cursor-pointer"
                  >
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="flex-[2] h-11 bg-primary text-primary-foreground text-xs rounded-full font-bold transition-all transform active:scale-95 shadow-lg shadow-primary/10 hover:brightness-105 flex items-center justify-center disabled:opacity-50 cursor-pointer"
                  >
                    {isLoading ? (
        ...
        {/* Footer links */}
        <footer className="w-full text-center text-[10px] text-text-secondary/70 mt-8 relative z-10 flex flex-col gap-2">
